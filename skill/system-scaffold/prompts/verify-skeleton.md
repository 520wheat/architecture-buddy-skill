...
在 SC5 之后加载。用于在最小骨架生成后执行最终门禁；只有验证证据齐全时才允许输出 `skeleton-ready`。
...
- 已确认的 technology-stack decision；
- skeleton manifest；
- 生成的骨架文件列表；
- unimplemented-items 列表；
- build / compile / test 相关命令及其最新输出；
...
2. 检查技术栈合规性：language/runtime、framework、dependency manager、build/start/test 入口是否与已确认 decision 一致。
...
7. 检查 compile / build 与 test entry：能运行的构建或编译命令必须成功；若技术栈支持测试入口，则测试入口必须可达并被明确记录。
8. 检查 unimplemented-items 与 skeleton manifest：所有未完成业务行为都要可见，且生成文件清单必须完整列出每个文件及其用途。
9. 如果缺少确认栈字段、manifest 不完整、或验证证据不足，保持 `draft`；如果出现架构/设计/栈冲突，输出 `blocked` 并说明回退去向；只有全部必需检查通过时才输出 `skeleton-ready`。
