# 架构设计：分布式内容寻址版本历史——工作树、索引、对象图与仓库同步
...
> 待验证事实：特定部署下 pack/repack、commit-graph、bitmap、partial clone 的容量与延迟曲线；远程宿主提供的认证、授权和高可用策略
...
成功时，用户可以在没有远程服务器的情况下完成一次本地 commit；另一个分支可以独立前进；两条历史可以合并；需要分享时，仓库只交换对方缺少的对象并更新约定的 refs。Git 不把远程服务器作为本地历史成立的前提，也不把分支实现成整棵工作树的复制品。
...
- 远程仓库通过 Git 协议或其他传输通道交换 refs 与 pack；它是分享和协作节点，不是本地 commit 的必经控制面。
...
| 组件 | 职责 | 关键契约 |
|------|------|----------|
| Working Tree | 承载用户可编辑文件 | 可与仓库历史暂时不一致；未提交状态不等于对象历史 |
...
| Pack / Index / Commit Graph | 降低对象存储、查找、遍历和传输成本 | 这些是可替换的物理加速层，不改变 blob/tree/commit 的逻辑模型 |
| Plumbing / Porcelain | 分离底层对象原语和面向用户的工作流 | 高层命令可组合底层能力；脚本可以依赖稳定的 plumbing 契约 |

禁止越界：不要把 pack 当作另一种历史语义；不要把远程 push 写成 commit 的同义词；不要用命令目录或 C 文件数量代替上述状态与契约。

### A5 状态、失败与恢复

持久化状态包括对象库中的 loose objects 或 packs、refs/HEAD、reflog、索引、配置、工作树关联信息和可选的 commit-graph、bitmap、MIDX、shallow/promisor 元数据。对象通常先可以以 loose 形式落盘，再由 repack/gc 组织为 pack；压缩不能改变对象的逻辑身份。
...
- **现在：** 先保持内容寻址的 blob/tree/commit/tag、DAG 历史、refs/HEAD、工作树与 index 三态、离线 commit、可选 push/fetch 和 plumbing/porcelain 分层。
...
- **对照中央线性 VCS：** Git 采用本地可提交的对象库和多仓库分享，教训是“提交记录”和“向公共面发布”应拆成两个机制；但如果团队只需要单一中央工作流，Git 的灵活性确实会换来更高的学习与治理成本。
- **对照纯 delta 变更集：** Git 以快照对象承载逻辑语义，再在 pack 层使用 delta 压缩，教训是不要让存储优化反过来定义用户可见历史模型。
